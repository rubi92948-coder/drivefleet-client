...
const MyAddedCars = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      setCars((prev) =>
        prev.map((car) =>
          car._id === editCar._id ? { ...car, ...res.data } : car
        )
      );
    ...
    <div className="bg-[#020617] min-h-screen text-white p-4 sm:p-8 relative">
              ...
              >
                ...
                <div className="px-6 pb-6 pt-0 flex gap-3">
                  <button
                    ...
                    className="flex-1 border border-orange-500/30 hover:border-orange-500 bg-orange-500/5 py-2.5 rounded-xl text-sm font-semibold text-gray-200 transition"
                  >
                    Update
                  </button>
                  ...
                  <button
                    ...
                    className="flex-1 bg-red-600/10 hover:bg-red-600 border border-red-500/20 text-red-400 hover:text-white py-2.5 rounded-xl text-sm font-semibold transition"
                  >
                    Delete
                  </button>
                </div>
      ...
      {editCar && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-[#0f172a] border border-gray-800/80 p-6 sm:p-8 rounded-2xl w-full max-w-lg shadow-2xl">
            <h2 className="text-2xl font-extrabold mb-6 text-orange-500">
              Update Fleet Specs
            </h2>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <input
                className="w-full px-4 py-2.5 rounded-xl bg-black border border-gray-800 text-sm"
                value={editCar.name}
                onChange={(e) =>
            ...
            <div className="flex gap-4 mt-8">
              <button
                ...
                className="flex-1 border border-gray-800 text-gray-400 py-3 rounded-xl"
              >
                Discard
              </button>
              <button
                onClick={handleUpdate}
                className="flex-1 bg-orange-500 py-3 rounded-xl font-bold"
              >
                Save Changes
              </button>
            </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-[#0f172a] p-6 rounded-2xl w-full max-w-sm text-center">
            <h3 className="text-xl font-bold mb-4">Remove Listing?</h3>
            <div className="flex gap-3">
              <button
                onClick={() => setDeleteId(null)}
                className="flex-1 border border-gray-800 py-2.5 rounded-xl"
              >
                Cancel
              </button>
              <button
